...
NEURON {
	...
	RANGE ica, ina , itot, imax, allost, rate
...
BREAKPOINT {
	:LOCAL rate
    if (is_fixed==0) {
        rate = pumprate_allo(v,nai,nao,cai,cao)
    }
    if (is_fixed==1) {
        rate = pumprate_allo(v,naf,nao,cai,cao)
    }
	ina =  3*rate
	ica = -2*rate
	itot=ina+ica
...
FUNCTION pumprate_allo(v,nai,nao,cai,cao) {
	...
	pumprate_allo = (q10*imax*(Kqa*nai*nai*nai*cao-KB*nao*nao*nao*cai)/((kna*kna*kna + nao*nao*nao)*(kca + cao)*(1 + ksat*KB)))*allo
}
